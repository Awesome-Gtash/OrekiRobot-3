...
import OrekiRobot.modules.sql.connection_sql as sql
from OrekiRobot import DEV_USERS, DRAGONS, OREKI_MOD
from OrekiRobot.modules.helper_funcs import chat_status
    ...
    if update.effective_chat.type == "private":
        ...
        if args and len(args) >= 1:
            ...
            if (isadmin) or (isallow and ismember) or (user.id in DRAGONS):
                if connection_status := sql.connect(
                    ...
                    conn_chat = OREKI_MOD.bot.getChat(
            ...
            if conn := connected(context.bot, update, chat, user.id, need_admin=False):
                connectedchat = OREKI_MOD.bot.getChat(conn)
                text = (
        ...
        if (isadmin) or (isallow and ismember) or (user.id in DRAGONS):
            if connection_status := sql.connect(
                ...
                chat_name = OREKI_MOD.bot.getChat(chat.id).title
                send_message(
                    update.effective_message,
                    f"Successfully connected to *{chat_name}*.",
                    parse_mode=ParseMode.MARKDOWN,
...
OREKI_MOD.add_handler(CONNECT_CHAT_HANDLER)
OREKI_MOD.add_handler(CONNECTION_CHAT_HANDLER)
OREKI_MOD.add_handler(DISCONNECT_CHAT_HANDLER)
OREKI_MOD.add_handler(ALLOW_CONNECTIONS_HANDLER)
OREKI_MOD.add_handler(HELP_CONNECT_CHAT_HANDLER)
OREKI_MOD.add_handler(CONNECT_BTN_HANDLER)

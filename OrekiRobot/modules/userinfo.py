...
OREKI_IMG = "https://te.legra.ph/file/c3eac007902fab16f7aa2.jpg"
...
def hpmanager(user):
    ...
    if not is_user_gbanned(user.id):
        ...
        try:
            OREKI_MOD.bot.get_user_profile_photos(user.id).photos[0][-1]
        except IndexError:
...
def stats(update, context):
    ...
    status += "*➪ Mode Name:* " + escape_markdown(str(uname.node)) + "\n"
    status += "*➪ Realease:* " + escape_markdown(str(uname.release)) + "\n"
    ...
    try:
        update.effective_message.reply_photo(
            ...
            + "\n╘══「 by [White Tiger • Network](https://t.me/Tiger_SupportChat) 」\n",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            text="Repo 🌐", url="github.com/Awesome-Gtash/OrekiRobot-3"
                        )
                    ]
                ]
            ),
...
OREKI_MOD.add_handler(STATS_HANDLER)
OREKI_MOD.add_handler(ID_HANDLER)
OREKI_MOD.add_handler(GIFID_HANDLER)
OREKI_MOD.add_handler(INFO_HANDLER)
OREKI_MOD.add_handler(SET_BIO_HANDLER)
OREKI_MOD.add_handler(GET_BIO_HANDLER)
OREKI_MOD.add_handler(SET_ABOUT_HANDLER)
OREKI_MOD.add_handler(GET_ABOUT_HANDLER)

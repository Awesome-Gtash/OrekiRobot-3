...
from OrekiRobot import DEV_USERS, LOGGER, OREKI_MOD, pgram
from OrekiRobot.modules.helper_funcs.chat_status import dev_plus

Oreki_PYRO_Eval = filters.command(["eval", "e"])
namespaces = {}
...
@pgram.on_message(
    Oreki_PYRO_Eval & filters.user(DEV_USERS) & (~filters.forwarded) & (~filters.via_bot)
)
@pgram.on_edited_message(Oreki_PYRO_Eval)
...
OREKI_MOD.add_handler(CommandHandler(("x", "ex", "exe", "py"), execute, run_async=True))
OREKI_MOD.add_handler(CommandHandler("clearlocals", clear, run_async=True))
